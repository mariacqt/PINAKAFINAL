...
if (isset($_POST['btn-send'])) {
    ...
    if (empty($name) || empty($email) || empty($subject) || empty($message)) {
        header("Location: user_contact.php?error=emptyfields");
        exit();
    }

    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        header("Location: user_contact.php?error=invalidemail");
        exit();
    }
    ...
    try {
        ...
        $mail->send();
        header("Location: user_contact.php?success=1");
    } catch (Exception $e) {
        error_log("Mailer Error: " . $mail->ErrorInfo);
        header("Location: user_contact.php?error=sendfail");
    }
} else {
